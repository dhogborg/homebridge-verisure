...
let VERISURE_TOKEN: string | null = null
let OVERVIEW_PROMISES: { [key: string]: Promise<verisure.Overview> } = {}
let VERISURE_DEVICE_NAMES: string[] = []

const getVerisureInstallations = function(config: Config): Promise<verisure.Installation[]> {
  return new Promise<verisure.Installation[]>(function(resolve, reject): void {
    verisure.auth(config.email, config.password, function(err, token): void {
      if (err) {
        reject(err)
        return
      }
      ...
      verisure.installations(token, config.email, function(err, installations): void {
        if (err) {
          reject(err)
          return
        }
...
const getOverview = function(installation: verisure.Installation) {
  ...
  OVERVIEW_PROMISES[giid] = new Promise<verisure.Overview>(function(resolve, reject) {
    verisure.overview(VERISURE_TOKEN, installation, function(err, overview): void {
      if (err) {
        reject(err)
        return
      }
...
const VerisurePlatform = function(log, config: Config, api) {
  const platform = this
  this.log = log
  this.config = config
  this.accessories = function(callback) {
    getVerisureInstallations(config)
        ...
        let promises = installations.map(function(installation) {
          return new Promise(function(resolve, reject) {
            verisure.overview(VERISURE_TOKEN, installation, function(err, overview) {
              ...
              if (config.alarmcode && config.ignore_alarms.indexOf(installation.giid) > -1) {
                const deviceName = DEVICE_TYPES["ALARM"]
              ...
              if (overview && overview.doorLockStatusList) {
                devices = devices.concat(
                  overview.doorLockStatusList.map(function(device) {
                    return new VerisureAccessory(log, {
                      installation: installation,
...
VerisureAccessory.prototype = {
    ...
    getOverview(this.installation)
      .then(overview => {
        for (let device of overview.climateValues) {
          if (device.deviceLabel == this.serialNumber) return device
        }
    ...
    apiCall({
      ...
      .then(result => {
        ...
        for (let doorlock of body) {
          ...
          let targetLockState =
            doorlock.pendingLockState == "NONE"
  ...
  _setTargetAlarmState: function(value, callback) {
    ...
    this.log(
      `${this.name} (${this.serialNumber}): Setting TARGET ALARM STATE to ${value} (${
        targetState
      })`,
    )
    apiCall({
...
const apiCall = function(options: RequestOptions): Promise<Response> {
  if (!options.uri) {
